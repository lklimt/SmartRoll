/*
  ...
  RF implementation is copied from validated Test 5 / Test 3 and MUST NOT be changed.
  PIR is NOT part of SmartRoll.

  Serial Monitor: 115200 Bd
  Commands: u=UP, d=DOWN, s=STOP, p=STATUS, r=reset Hall, h=HELP
...
const char* FW_VERSION = "0.6.3-test6A3";
...
// IMPORTANT: RfSend is abstract. Use the exact builder configuration proven in Test 5.
...
void IRAM_ATTR hallISR() {
  ...
  transitionCount++;
  if (movement == 1) {
    position++; cwCount++; lastDirection = 1;
  } else if (movement == -1) {
    position--; ccwCount++; lastDirection = -1;
  } else if (movement == 2) {
    invalidCount++;
  }
...
void readSensors() {
  ...
  if (bh1750Ok) lux = lightMeter.readLightLevel();
  if (ds18b20Ok) {
...
void printStatus() {
  int32_t p; uint32_t cw, ccw, inv, tr; uint8_t state; int8_t dir;
  noInterrupts();
  p=position; cw=cwCount; ccw=ccwCount; inv=invalidCount; tr=transitionCount;
  state=previousState; dir=lastDirection;
  interrupts();
  Serial.println("--------------------------------------------");
  Serial.printf("Wi-Fi: %s", WiFi.status()==WL_CONNECTED ? "CONNECTED" : "OFFLINE");
  if (WiFi.status()==WL_CONNECTED) Serial.printf("  IP=%s RSSI=%d dBm", WiFi.localIP().toString().c_str(), WiFi.RSSI());
  Serial.println();
  Serial.printf("MQTT: %s\n", mqtt.connected() ? "CONNECTED" : "OFFLINE");
  Serial.printf("Hall: A=%d B=%d position=%ld CW=%lu CCW=%lu transitions=%lu invalid=%lu\n",
    (state>>1)&1, state&1, (long)p, (unsigned long)cw, (unsigned long)ccw, (unsigned long)tr, (unsigned long)inv);
...
void ensureMQTT() {
  ...
  bool ok;
  if (strlen(MQTT_USER)==0) ok=mqtt.connect(clientId.c_str(),availabilityTopic.c_str(),0,true,"offline");
  else ok=mqtt.connect(clientId.c_str(),MQTT_USER,MQTT_PASSWORD,availabilityTopic.c_str(),0,true,"offline");
  if (ok) {
...
void publishState() {
  ...
  int32_t p; uint32_t cw,ccw,inv,tr; uint8_t state; int8_t dir;
  noInterrupts(); p=position; cw=cwCount; ccw=ccwCount; inv=invalidCount; tr=transitionCount; state=previousState; dir=lastDirection; interrupts();
  String payload="{";
...
void printHelp() { Serial.println("Commands: u=UP d=DOWN s=STOP p=STATUS r=reset Hall h=HELP"); }
...
void handleSerial() {
  while (Serial.available()) {
    ...
    if(c=='u'||c=='U') sendRF("UP","01C",data_up,sizeof(data_up),UP);
      ...
      noInterrupts(); position=0; cwCount=0; ccwCount=0; invalidCount=0; transitionCount=0; lastDirection=0; interrupts();
...
void setup() {
  ...
  // EXACT proven RF builder from Test 5.
  ...
  // Wi-Fi sequence proven by Test 6A.2.
  WiFi.mode(WIFI_STA);
  WiFi.disconnect();
  delay(300);
  WiFi.begin(WIFI_SSID,WIFI_PASSWORD);
  ...
  Serial.println();
  if(WiFi.status()==WL_CONNECTED) Serial.printf("Wi-Fi: CONNECTED IP=%s RSSI=%d dBm\n",WiFi.localIP().toString().c_str(),WiFi.RSSI());
  else Serial.printf("Wi-Fi: initial connection failed, status=%d\n",(int)WiFi.status());
